...
if (!window.__EARLY_BIRD_PRICE_ADJUSTMENT_LOADED__) {
    ...
    // Global instance - initialize when Bootstrap is ready
    function initializeEarlyBirdPriceAdjustment() {
        if (typeof bootstrap !== 'undefined') {
            window.earlyBirdPriceAdjustment = new EarlyBirdPriceAdjustment();
            console.log('[EarlyBird] Initialized successfully');
        } else {
            console.warn('[EarlyBird] Bootstrap not yet available, retrying in 100ms...');
            setTimeout(initializeEarlyBirdPriceAdjustment, 100);
        }
    }

    // Initialize on DOM ready or immediately if already loaded
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeEarlyBirdPriceAdjustment);
    } else {
        initializeEarlyBirdPriceAdjustment();
    }
    ...
    window.checkAndShowPriceAdjustment = async function (enrollmentId, onConfirmCallback = null) {
        ...
        // Wait for earlyBirdPriceAdjustment to be initialized
        let retries = 0;
        while (!window.earlyBirdPriceAdjustment && retries < 20) {
            console.log('[EarlyBird] waiting for initialization...');
            await new Promise(resolve => setTimeout(resolve, 50));
            retries++;
        }

        if (!window.earlyBirdPriceAdjustment) {
            console.error('[EarlyBird] failed to initialize, skipping dialog');
            if (onConfirmCallback) {
                onConfirmCallback({ adjustment_made: false, error: 'initialization_failed' });
            }
            return false;
        }
